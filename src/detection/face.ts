...
export class FaceGate {
  ...
  /**
   * `WAIT_FOR_PARTNER` — "I'll go get them". Clears the solo accumulator and
   * re-arms the one-shot so the prompt can fire again if they come back alone,
   * WITHOUT clearing the latch buffers (Doc 02 §2.10).
   */
  resetSolo(): void {
    this.soloElapsedMs = 0;
