...
export function evaluateG1(input: G1Input): VariantResult {
  ...
  const thumbLimit = upperBound(T.thumbJunction * sBar * m, active);
  const c2: Condition = {
    id: 'C2',
  ...
  const indexLimit = upperBound(T.indexJunction * sBar * m, active);
  const c3: Condition = {
    id: 'C3',
  ...
  const apertureLimit = lowerBound((T.aperture * sBar) / m, active);
  const c5: Condition = {
    id: 'C5',
    label: 'aperture',
